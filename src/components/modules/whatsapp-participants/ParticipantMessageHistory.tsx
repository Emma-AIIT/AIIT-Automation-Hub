...
import type { ParticipantMessageLogEntry } from '@/server/api/routers/whatsapp';
...
const STATUS_STYLES: Record<ParticipantMessageLogEntry['status'], { dot: string; label: string; badge: string }> = {
...
interface ParticipantMessageHistoryProps {
  accountId: WhatsAppAccountId;
  refreshBump?: number; // bump to trigger a refetch after a new send
}

export function ParticipantMessageHistory({ accountId, refreshBump }: ParticipantMessageHistoryProps) {
  const [expandedErrors, setExpandedErrors] = useState<Set<string>>(new Set());
  const [expandedRecipients, setExpandedRecipients] = useState<Set<string>>(new Set());
  const [page, setPage] = useState(1);

  const { data: history = [], isLoading, refetch } = api.whatsapp.listParticipantMessageHistory.useQuery(
    { accountId },
    { refetchInterval: 60_000 },
  );

  // Refetch when parent signals a new send just completed
  ...
  // Back to first page when switching account
  useEffect(() => {
    setPage(1);
  }, [accountId]);
    ...
    <div className="rounded-xl border border-(--color-border-subtle) bg-white shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-4 border-b border-(--color-border-subtle) flex items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-semibold text-(--color-text-primary)">Message History</h2>
            {history.length > 0 && (
          ...
          <p className="text-xs text-(--color-text-muted) mt-0.5">Log of messages sent individually from this page</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="px-5 py-10 flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-sm text-(--color-text-muted)">No messages sent yet</p>
        ...
        <div className="divide-y divide-(--color-border-subtle)">
          {pagedHistory.map((entry) => {
            ...
            const isRecipientsExpanded = expandedRecipients.has(entry.id);
            // Prefer names, fall back to phones, for the label list
            const labels = (entry.recipient_names?.length ? entry.recipient_names : entry.recipient_phones) ?? [];
            const recipientCount = entry.recipient_ids?.length || labels.length;
            const hasMoreRecipients = labels.length > 2;
            ...
            const recipientSummary = labels.slice(0, 2).join(', ');
              ...
              <div key={entry.id} className="px-5 py-3.5 hover:bg-(--color-bg-secondary) transition-colors">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                      ...
                      <span className="text-xs text-(--color-text-muted)">
                        ...
                        {hasMoreRecipients && (
                          <>
                            {', '}
                            <button
                              onClick={() => toggleRecipients(entry.id)}
                    ...
                    {hasMoreRecipients && isRecipientsExpanded && (
      ...
      {history.length > HISTORY_PAGE_SIZE && (
        <div className="flex items-center justify-between gap-4 px-5 py-3 border-t border-(--color-border-subtle) bg-(--color-bg-secondary)">
          <p className="text-xs text-(--color-text-muted)">
            Showing {(currentPage - 1) * HISTORY_PAGE_SIZE + 1}–{Math.min(currentPage * HISTORY_PAGE_SIZE, history.length)} of {history.length}
          </p>
          <div className="flex items-center gap-2">
            <button
              type="button"
              ...
              title="Newer"
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-lg border border-(--color-border-default) bg-white text-(--color-text-secondary) hover:bg-(--color-bg-hover) disabled:opacity-50 disabled:cursor-not-allowed transition"
            >
